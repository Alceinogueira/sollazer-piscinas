...
router.post(
  ...
  async (req, res) => {
    ...
    const { nome, descricao, marca, categoria, preco, estoque, imagem_url, tipo_acao } = req.body;
    ...
    try {
      ...
      const [result] = await db.execute(
        `INSERT INTO produtos (nome, descricao, marca, categoria, preco, estoque, imagem_url, tipo_acao)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [nome, descricao || null, marca || null, categoria || null, preco, estoque || 0, imagem_url || null, tipo_acao]
      );
...
router.put(
  ...
  [
    ...
    body('estoque').optional().isInt({ min: 0 }),
    body('imagem_url').optional().trim().isURL(),
  ...
  async (req, res) => {
    ...
    const { nome, descricao, marca, categoria, preco, estoque, imagem_url, tipo_acao, ativo } = req.body;

    try {
      const [result] = await db.execute(
        `UPDATE produtos
         SET nome = ?, descricao = ?, marca = ?, categoria = ?, preco = ?,
             estoque = ?, imagem_url = ?, tipo_acao = ?, ativo = ?
         WHERE id = ?`,
        [nome, descricao || null, marca || null, categoria || null, preco,
         estoque || 0, imagem_url || null, tipo_acao, ativo ?? true, id]
      );
